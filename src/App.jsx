  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0d0d1a", fontFamily:"sans-serif", color:"#fff" }}>
      ...
      <div style={{ padding:24, overflow:"auto" }}>
        {tab==="dashboard"  && <Dashboard sales={sales} receipts={receipts} products={products} stockMap={stockMap} clients={clients} />}
        {tab==="clients"    && <ClientsView clients={clients} sales={sales} onAddClient={onAddClient} onUpdateClient={onUpdateClient} />}
...
// ── DASHBOARD ────────────────────────────────────────────────────────────────
function Dashboard({ sales, receipts, products, stockMap, clients }) {
  const now = new Date();
  ...
  const planRevenue = filtered.reduce((a,s)=>a+s.items.filter(i=>i.type==="plan").reduce((x,i)=>x+i.finalPrice*i.qty,0),0);
  const productRevenue = filtered.reduce((a,s)=>a+s.items.filter(i=>i.type==="product").reduce((x,i)=>x+i.price*i.qty,0),0);

  const itemMap = {};
  filtered.forEach(s=>s.items.forEach(i=>{ if(!itemMap[i.name])itemMap[i.name]={revenue:0,qty:0,type:i.type}; itemMap[i.name].revenue+=(i.finalPrice||i.price)*i.qty; itemMap[i.name].qty+=i.qty; }));
  const topItems = Object.entries(itemMap).sort((a,b)=>b[1].revenue-a[1].revenue).slice(0,6);

  const daysInMonth = new Date(parseInt(selectedMonth.split("-")[0]),parseInt(selectedMonth.split("-")[1]),0).getDate();
  const dailyData = Array.from({length:daysInMonth},(_,i)=>{ const day=String(i+1).padStart(2,"0"); const ds=filtered.filter(s=>s.date.slice(0,10)===`${selectedMonth}-${day}`); return {day:i+1,total:ds.reduce((a,s)=>a+s.total,0)}; });
  const maxDay = Math.max(...dailyData.map(d=>d.total),1);
  const lowStock = products.filter(p=>(stockMap[p.id]||0)<5);

  // Plan expiry alerts (next 10 days)
  const expiringPlans = [];
  sales.forEach(s => {
    s.items.forEach(it => {
      if (it.type==="plan" && it.endDate) {
        const d = daysUntil(it.endDate);
        if (d >= 0 && d <= 10) {
          const cl = clients.find(c=>c.id===s.clientId);
          expiringPlans.push({ clientName:s.clientName||"—", planName:it.name, endDate:it.endDate, daysLeft:d, saleDate:s.date });
        }
      }
    });
  });
  // dedupe by clientName+planName keeping soonest
  const seen = {};
  const alerts = [];
  expiringPlans.sort((a,b)=>a.daysLeft-b.daysLeft).forEach(e => {
    const k = `${e.clientName}_${e.planName}`;
    if (!seen[k]) { seen[k]=true; alerts.push(e); }
  });
        ...
        <div style={{ background:"#1a1220", border:"1px solid #9b59b6", borderRadius:10, padding:"12px 16px", marginBottom:16 }}>
          ...
          {alerts.map((a,i) => (
            <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"6px 0", borderBottom:i<alerts.length-1?"1px solid #2a1a3a":"none" }}>
              <div>
                <span style={{ fontWeight:600, marginRight:8 }}>{a.clientName}</span>
                <span style={{ color:"#aaa", fontSize:13 }}>{a.planName}</span>
              </div>
...
// ── CLIENTS VIEW ─────────────────────────────────────────────────────────────
function ClientsView({ clients, sales, onAddClient, onUpdateClient }) {
  const [search, setSearch] = useState("");
  ...
  const activePlans = useMemo(() => {
    ...
    clientSales.forEach(s => s.items.forEach(it => {
      if (it.type==="plan" && it.endDate) {
        const d = daysUntil(it.endDate);
        plans.push({ ...it, saleDate:s.date, daysLeft:d });
      }
    }));
    return plans.sort((a,b) => new Date(b.saleDate)-new Date(a.saleDate));
  }, [clientSales]);
